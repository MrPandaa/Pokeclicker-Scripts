...
class EnhancedAutoClicker {
  ...
  static initAutoClicker() {
    ...
    elemAC.innerHTML = `<tbody>
        <tr>
            <td colspan="4">
                <button id="auto-click-start" class="btn btn-${
                  this.autoClickState() ? 'success' : 'danger'
                } btn-block" style="font-size:8pt;">
                ...
                <div id="click-rate-cont">
                    ...
                    <input id="auto-click-rate" type="range" min="1" max="${
                      this.maxClickMultiplier
                    }" value="${this.autoClickMultiplier}">
                </div>
            </td>
        </tr>
        <tr>
            <td style="display: flex; column-gap: 2px;">
                <div style="flex: auto;">
                    <button id="auto-dungeon-start" class="btn btn-block btn-${
                      this.autoDungeonState() ? 'success' : 'danger'
                    }" style="font-size: 8pt;">
                ...
                <div id="auto-dungeon-encounter-mode" style="flex: initial; max-height: 30px; max-width: 30px; padding: 2px;">
                    <img title="Auto Dungeon fights mode" src="assets/images/dungeons/encounter.png" height="100%" style="${
                ...
                <div id="auto-dungeon-chest-mode" style="flex: initial; max-height: 30px; max-width: 40px; padding: 2px;">
                    <img title="Auto Dungeon chest mode" src="assets/images/dungeons/chest.png" height="100%" style="${
                      this.autoDungeonChestMode ? '' : 'filter: grayscale(100%)'
                    }" />
                </div>
                <div style="flex: initial; display: flex; flex-direction: column;">
                    <div id="auto-dungeon-loottier" class="dropdown show">
                        <button type="button" class="text-left custom-select col-12 btn btn-dropdown" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="max-height:30px; display:flex; flex:1; align-items:center;">
                            <img id="auto-dungeon-loottier-img" src="assets/images/dungeons/chest-${
                              Object.keys(baseLootTierChance)[
                                this.autoDungeonLootTier
                              ]
                        ...
                        <div id="auto-dungeon-loottier-dropdown" class="border-secondary dropdown-menu col-12">
                            ${Object.keys(baseLootTierChance)
                              .map(
                                (tier, i) =>
                                  `<div class="dropdown-item" value="${i}"><img src="assets/images/dungeons/chest-${tier}.png"></div>`
                              )
                              .join('\n')}
                        </div>
                    </div>
                ...
                <div style="flex: auto;">
                    <button id="auto-gym-start" class="btn btn-block btn-${
                      this.autoGymState() ? 'success' : 'danger'
                    }" style="font-size: 8pt;">
                        Auto Gym [${this.autoGymState() ? 'ON' : 'OFF'}]
                    </button>
                </div>
                <div style="flex: initial; display: flex; flex-direction: column;">
                    <select id="auto-gym-select" value="${
                      this.autoGymSelect
                    }" style="flex: auto;">
        ...
        <tr>
            <td style="display: flex; column-gap: 2px;">
                <div style="flex: auto;">
                    <button id="auto-achievement-start" class="btn btn-block btn-${
                      this.autoAchievementState() ? 'success' : 'danger'
                    }" style="font-size: 8pt;">
                            Auto Achievement [${
                              this.autoAchievementState() ? 'ON' : 'OFF'
                            }]
    ...
    dropdownsToAdd.forEach(([name, text, value, options]) => {
      ...
      newSetting.innerHTML = `<td class="p-2 col-md-8">
                ...
                ${options
                  .map((desc, val) => `<option value="${val}">${desc}</option>`)
                  .join('\n')}
                </select>
                </td>`;
    });
  ...
  static resetCalculator() {
    ...
    document.getElementById('auto-click-info').innerHTML = `<div>${
      this.autoClickCalcEfficiencyDisplayMode == 0
        ? 'Clicker Efficiency'
        : 'Ticks/s'
    }:<br><div id="tick-efficiency" style="font-weight:bold;">-</div></div>
            <div>${
              this.autoClickCalcDamageDisplayMode == 0
                ? 'Click Attacks/s'
                : 'DPS'
            }:<br><div id="clicks-per-second" style="font-weight:bold;">-</div></div>
            <div>Req. ${
              this.autoClickCalcDamageDisplayMode == 0
                ? 'Clicks'
                : 'Click Damage'
            }:<br><div id="req-clicks" style="font-weight:bold;">-</div></div>
